...
const ListeningSection: React.FC = () => {
  ...
  const render = (data: ListeningTestItem[]) => {
    ...
    setFilteredListenings(sortedData);
  ...
  const handleSearch = (query: string) => {
    ...
    if (!lowerCaseQuery) {
      render(listening); // Reset to all readings if query is empty
      return;
    }

    const filtered = listening.filter((test) =>
  ...
  const filterByPartCount = (count: string | null) => {
    ...
    const filtered = listening.filter(
